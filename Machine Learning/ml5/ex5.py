...
class CNNModel(Module):
    # def __init__(self, _shape_x, _shape_y, _stride, _kernel_size, _padding, _kernel_depth, _lr):
    def __init__(self, _shape_x, _shape_y, _lr, params):
        super().__init__()

        self.cnn_layers = Sequential(
            Conv2d(1, params['conv1']['kernel_depth'], kernel_size=params['conv1']['kernel_size'],
                   stride=params['conv1']['stride'], padding=params['conv1']['padding'], device=DEVICE),
            BatchNorm2d(params['conv1']['kernel_depth'], device=DEVICE),
            ReLU(inplace=True),
            Dropout(params['dropout1']['p']),
            AvgPool2d(params['avgpool1']['kernel_size'],
                      params['avgpool1']['stride']),
            # MaxPool2d(params['maxpool1']['kernel_size'],
            #           params['maxpool1']['stride']),

            Conv2d(params['conv1']['kernel_depth'], params['conv2']['kernel_depth'], kernel_size=params['conv2']['kernel_size'],
                   stride=params['conv2']['stride'], padding=params['conv2']['padding'], device=DEVICE),
            BatchNorm2d(params['conv2']['kernel_depth'], device=DEVICE),
            ReLU(inplace=True),
            Dropout(params['dropout2']['p']),
            AvgPool2d(params['avgpool2']['kernel_size'],
                      params['avgpool2']['stride']),
            # MaxPool2d(params['maxpool2']['kernel_size'],
            #   params['maxpool2']['stride']),

            Conv2d(params['conv2']['kernel_depth'], params['conv3']['kernel_depth'], kernel_size=params['conv3']['kernel_size'],
                   stride=params['conv3']['stride'], padding=params['conv3']['padding'], device=DEVICE),
            BatchNorm2d(params['conv3']['kernel_depth'], device=DEVICE),
            ReLU(inplace=True),
            Dropout(params['dropout2']['p']),
            AvgPool2d(params['avgpool3']['kernel_size'],
                      params['avgpool3']['stride']),
            # MaxPool2d(params['maxpool3']['kernel_size'],
            #           params['maxpool3']['stride']),

            # Conv2d(_kernel_depth, _kernel_depth, kernel_size=_kernel_size,
            #        stride=_stride, padding=_padding, device=DEVICE),
            # BatchNorm2d(_kernel_depth, device=DEVICE),
            # ReLU(inplace=True),
            # MaxPool2d(kernel_size=2, stride=2),
        )

        # input 161 x 101 x 1
        # N = size of input + padding, F = kernel size
        # size after conv = (N - F) / stride + 1
        # _shape_x = (_shape_x + 2 * params['conv1']['padding'] -
        #             params['conv1']['kernel_size']) / params['conv1']['stride'] + 1
        # _shape_y = (_shape_y + 2 * params['conv1']['padding'] -
        #             params['conv1']['kernel_size']) / params['conv1']['stride'] + 1
        # _shape_x = int((_shape_x - params['maxpool1']['kernel_size']) / params['maxpool1']['stride'] + 1)
        # _shape_y = int((_shape_x - params['maxpool1']['kernel_size']) / params['maxpool1']['stride'] + 1)
        # _shape_x = (_shape_x + 2 * params['conv2']['padding'] -
        #             params['conv2']['kernel_size']) / params['conv2']['stride'] + 1
        # _shape_y = (_shape_y + 2 * params['conv2']['padding'] -
        #             params['conv2']['kernel_size']) / params['conv2']['stride'] + 1
        # _shape_x = int((_shape_x - params['maxpool2']['kernel_size']) / params['maxpool2']['stride'] + 1)
        # _shape_y = int((_shape_x - params['maxpool2']['kernel_size']) / params['maxpool2']['stride'] + 1)
        self.linear_input_shape = int(
            _shape_x * _shape_y * params['conv2']['kernel_depth'])

        self.linear_input_shape = self.cnn_layers(torch.zeros(
            1, 1, _shape_x, _shape_y, device=DEVICE)).numel()
        self.linear_layers = Sequential(
            Linear(self.linear_input_shape,
                   params['linear1']['out'], device=DEVICE),
            Linear(params['linear1']['out'],
                   params['linear2']['out'], device=DEVICE),
            Linear(params['linear2']['out'],
                   params['linear3']['out'], device=DEVICE),
        )

        self.optimizer = torch.optim.SGD(self.parameters(), lr=_lr)
...
def compare_to_test_labels(results, class_to_idx):
    with open('test_labels_sorted', 'r') as labels:
        labels_arr = np.array(
            [class_to_idx[line.strip('\n').split(',')[1]] for line in labels.readlines()])
        correct = (np.array(labels_arr, dtype=int) == np.array(
            [x[1].item() for x in results], dtype=int)).sum()
        print(f'Test: {correct} out of {len(labels_arr)}, ({correct * 100 / len(labels_arr)}%)')
...
def main():
    ...
    import json
    params_file = os.path.join('params', 'resnet_params.json')
    # params_file = os.path.join('params', 'cnn_params.json')
    with open(params_file, 'r') as params_json:
        params = json.load(params_json)
    # model = CNNModel(_shape_x, _shape_y, _lr, params)
    ...
    # test
    _, correct, results = test(model, test_loader, with_results=True)
    classes, class_to_idx = find_classes(os.path.join(path_to_data, 'train'))
    compare_to_test_labels(results, class_to_idx)
    with open('test_y', 'w') as testy:
        for path, pred in results:
            testy.write(f'{path},{classes[pred.item()]}\n')
            
    # for testing only
    _, correct, results = test(model, train_loader, with_results=True)
    print(len(train_loader.dataset))
    print(f'Train: {correct} out of {len(train_loader.dataset)}, ({correct * 100 / len(train_loader.dataset)}%)')
    _, correct, results = test(model, val_loader, with_results=True)
    print(len(val_loader.dataset))
    print(
        f'Validation: {correct} out of {len(val_loader.dataset)}, ({correct * 100 / len(val_loader.dataset)}%)')
...
if __name__ == '__main__':
    ...
    print(
        f'Measured time: {end - start} seconds, which is {(end - start) // 60} minutes and {(end - start) % 60} seconds.')
